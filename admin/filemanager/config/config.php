...
// mb_http_input('UTF-8'); // Deprecated in PHP 8.1 - removed
mb_language('uni');
...
if ($_SERVER['HTTP_HOST'] == 'localhost' || strpos($_SERVER['HTTP_HOST'], 'localhost:') === 0) {
    ...
    // HIEP-CMS-8 PHP 8.1 FIX: Better detection for localhost with port
    // Handle localhost:8080/HIEP-CMS-8/admin/filemanager/... structure
    if (count($arr_url) >= 2 && !empty($arr_url[1])) {
        $forder = $arr_url[1]; // Gets "HIEP-CMS-8" from URL
    } else {
        $forder = '';
    }
} else {
