...
class imageLib {
	...
	private $debug      = false;  // HIEP-CMS-8 PHP 8.1 Fix: Disable debug to prevent false error messages
...
## --------------------------------------------------------
		...
		#       third party class to save as bmp.
	{
		...
		// HIEP-CMS-8 PHP 8.1 Fix: Enhanced image resource validation
		$isValidImage = false;
		if (is_resource($this->imageResized)) {
			$isValidImage = true;  // Traditional resource for PHP < 8.0
		} elseif (class_exists('GdImage') && $this->imageResized instanceof GdImage) {
			$isValidImage = true;  // GdImage object for PHP 8.0+
		} elseif (is_object($this->imageResized) && get_class($this->imageResized) === 'GdImage') {
			$isValidImage = true;  // Fallback check for GdImage
		}
		
		if (!$isValidImage)
		{
			$imageType = is_object($this->imageResized) ? get_class($this->imageResized) : gettype($this->imageResized);
			error_log("HIEP-CMS-8: saveImage failed - imageResized is not a valid image resource. Type: " . $imageType);
			
			// HIEP-CMS-8 PHP 8.1 Fix: Don't throw exceptions for resource validation issues
			// This prevents false error messages while thumbnails are actually created successfully
			if ($this->debug)
			{
				error_log("HIEP-CMS-8: Debug mode - would have thrown exception for invalid image resource");
			}
...
## --------------------------------------------------------
		...
		#
	{
		...
		{
			$imageType = is_object($this->imageResized) ? get_class($this->imageResized) : gettype($this->imageResized);
			error_log("HIEP-CMS-8: displayImage failed - imageResized is not a valid image resource. Type: " . $imageType);
...
## --------------------------------------------------------
	...
	private function GetPixelColor(&$img, $x, $y)
		...
		#
	{
		// HIEP-CMS-8 PHP 8.1 Fix: Enhanced image resource validation for GetPixelColor
		$isValidImage = false;
		if (is_resource($img)) {
			$isValidImage = true;  // Traditional resource for PHP < 8.0
		} elseif (class_exists('GdImage') && $img instanceof GdImage) {
			$isValidImage = true;  // GdImage object for PHP 8.0+
		} elseif (is_object($img) && get_class($img) === 'GdImage') {
			$isValidImage = true;  // Fallback check for GdImage
		}
...
## --------------------------------------------------------
	...
	{
		// HIEP-CMS-8 PHP 8.1 Fix: Enhanced image resource cleanup
		$isValidImage = false;
		if (is_resource($this->imageResized)) {
			$isValidImage = true;  // Traditional resource for PHP < 8.0
		} elseif (class_exists('GdImage') && $this->imageResized instanceof GdImage) {
