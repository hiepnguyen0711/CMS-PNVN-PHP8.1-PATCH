...
class UploadHandler
{
        ...
        $this->options = array(
            ...
            */
            ...
            'image_versions' => array(
		        ...
		        */
                // Thumbnail configuration will be provided by upload.php
                // when properly configured through filemanager config
            ),
            'print_response' => true
        );
        if ($options) {
    ...
    protected function gd_get_image_object($file_path, $func, $no_cache = false) {
        if (empty($this->image_objects[$file_path]) || $no_cache) {
            ...
            // PHP 8.1 Fix: Add error handling for image creation
            $image = @$func($file_path);
            if (!$image) {
                error_log("Failed to create image from: " . $file_path . " using function: " . $func);
                return false;
            }
    ...
    protected function gd_destroy_image_object($file_path) {
        ...
        // PHP 8.1 Fix: Updated for GdImage object compatibility
    ...
    protected function gd_create_scaled_image($file_name, $version, $options) {
        ...
        // PHP 8.1 Fix: Validate image resource before processing
        if (!$src_img || (!is_resource($src_img) && !($src_img instanceof GdImage))) {
            error_log("HIEP-CMS-8: Failed to create image resource for: " . $file_path);
            return false;
        }
        ...
        if (!empty($options['auto_orient']) && $this->gd_orient_image(
            ...
            // PHP 8.1 Fix: Re-validate after orientation
            if (!$src_img || (!is_resource($src_img) && !($src_img instanceof GdImage))) {
                error_log("HIEP-CMS-8: Failed to re-create image resource after orientation for: " . $file_path);
                return false;
            }
        }
        ...
        // PHP 8.1 Fix: Store new image and clean up properly
        $this->gd_set_image_object($file_path, $new_img);
        
        // Clean up the new image resource after saving
        if ($new_img && (is_resource($new_img) || ($new_img instanceof GdImage))) {
    ...
    public function onUploadEnd ($res){
        ...
        if ($is_img)
        {
            ...
            // Thumbnail Fix 4: Enhanced thumbnail creation with better error handling
            $thumbResult = create_img($targetFile, $targetFileThumb, 122, 91, 'crop', $this->options['config']);
            ...
            if ( $thumbResult !== true)
            {
                if($thumbResult === false){
                    ...
                    // Thumbnail Fix 4: Log detailed error for debugging
                    error_log("Thumbnail creation failed: " . $thumbResult . " for file: " . $targetFile);
                    $res['files'][0]->error = "Thumbnail creation failed: " . $thumbResult;
                }
            }
            else
            {
