...
function create_img($imgfile, $imgthumb, $newwidth, $newheight = null, $option = "crop", $config = array())
{
	...
	if ((file_exists($imgfile) || strpos($imgfile, 'http') === 0) && $imgfile !== '') {
		$memory_ok = true;
		if (strpos($imgfile, 'http') !== 0) {
			$memory_ok = image_check_memory_usage($imgfile, $newwidth, $newheight);
		}
		
		if ($memory_ok || strpos($imgfile, 'http') === 0) {
			...
			try {
				if (!file_exists($imgfile) && strpos($imgfile, 'http') !== 0) {
					throw new Exception("Source image file does not exist: " . $imgfile);
				}
				
				$magicianObj = new imageLib($imgfile);
				
				if (!$magicianObj) {
					throw new Exception("Failed to create image object from: " . $imgfile);
				}
				
				$magicianObj->resizeImage($newwidth, $newheight, $option);
				
				$thumb_dir = dirname($imgthumb);
				if (!is_dir($thumb_dir)) {
				...
				if (!file_exists($imgthumb)) {
					throw new Exception("Failed to save thumbnail: " . $imgthumb);
				}
				
				$result = true;
			} catch (Exception $e) {
				error_log("Thumbnail creation error: " . $e->getMessage() . " for file: " . $imgfile);
				return $e->getMessage();
			}
		} else {
		...
		return "Source image file not found or invalid: " . $imgfile;
	}
	
	if ($result && isset($config['ftp_host']) && $config['ftp_host'] && $ftp) {
		try {
			if (!$ftp->put($save_ftp, $imgthumb, FTP_BINARY)) {
				throw new Exception("Failed to upload thumbnail via FTP");
			}
			unlink($imgthumb);
		} catch (Exception $e) {
...
function folder_info($path, $count_hidden = true)
{
	...
	$total_size = 0;
	$files = scandir($path);
	// PHP 8.1 compatibility: check if $files is array before foreach()
	if (!is_array($files)) {
		$files = array();
	}
 ...
 * @param  string  $ext
 */
// Thumbnail Fix 3: Fixed function signature and parameter handling 
function check_files_extensions_on_path($path, $ext)
{
	if (! is_dir($path)) {
		$fileinfo = pathinfo($path);
		if (isset($fileinfo['extension']) && ! in_array(mb_strtolower($fileinfo['extension']), $ext)) {
			unlink($path);
		}
	} else {
		$files = scandir($path);
...
{
	$check = false;
	// Thumbnail Fix 2: Fixed variable name from $conf to $config
...
function check_files_extensions_on_phar($phar, &$files, $basepath, $config)
{
	foreach ($phar as $file) {
		if ($file->isFile()) {
			// Thumbnail Fix 3: Fixed function call with proper parameters
