    ...
    switch ($_GET['action']) {
		...
		case 'save_img':
            ...
            if ($ftp) {
                ...
                // Thumbnail Fix 5: Enhanced thumbnail creation with error logging
                $thumbResult = create_img($config['current_path'] . $_POST['path'] . $_POST['name'], $config['thumbs_base_path'].$_POST['path'].$_POST['name'], 122, 91, 'crop', $config);
                if ($thumbResult !== true) {
                    error_log("AJAX thumbnail creation failed: " . $thumbResult . " for file: " . $_POST['name']);
                }
                
                // Thumbnail Fix 5: Enhanced new thumbnails creation with error logging  
                $additionalResult = new_thumbnails_creation(
                ...
                if (!$additionalResult) {
                    error_log("Additional thumbnail creation failed for file: " . $_POST['name']);
                }
            }
            break;
